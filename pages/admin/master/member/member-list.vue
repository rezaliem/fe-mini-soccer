<template>
  <NuxtLayout name="blank">
    ...
    >
      ...
      <AdminMasterMemberList class="mt-2" origin-mode="master-member" />
    </div>
  </NuxtLayout>
</template>
...
const isAllowedCreateMember = computed(() => {
  const grantedList = UserStoreInstance.getUserActiveGranted();
  return grantedList.find((action) => action === "insertMasterMember")
    ? true
    : false;
});

const createMasterHandler = async () => {
  await navigateTo("/admin/master/member/new");
};
</script>
